        ...
        times.forEach((link) => {
            link.addEventListener("click", (e) => {
                ...
                if ($currentRip && $player && $currentRip.ytid === rip.ytid) {
                    $player.seekTo(
                        parseInt(link.innerText.split(":")[0]) * 60 + parseInt(link.innerText.split(":")[1])
                    );
                } else {
                    currentRip.set(rip);
...
<div class="joke" bind:this={elem} class:active={rip === $currentRip} class:hidden={!$options.showJokes}>
