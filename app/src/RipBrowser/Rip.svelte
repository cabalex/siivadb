...
<div
    class="rip"
    on:click={toggleItem.bind(null, rip)}
...
>
    ...
    <div class="rip-inner">
        <div class="thumb">
            <img src={`https://img.youtube.com/vi/${rip.ytid}/sddefault.jpg`} />
        ...
        <div class="title">
            ...
            <span
                class="series"
                on:click={(e) => {e.stopPropagation(); searchType = "titles"; searchValue = rip.series || ""}}
            >
                {rip.series || ""}
            </span> - 
            <DateView date={rip.postTime} />
        </div>
        <Joke bind:searchValue={searchValue} bind:searchType={searchType} rip={rip} />
        <div class="rip-actions" on:click={(e) => e.stopPropagation()}>
            <button disabled style="color: #777">
                <PlaylistPlus />
            </button>
            <button class:copied={copied === rip.ytid} on:click={copyLink.bind(null, rip.ytid)}>
                <ContentCopy />
            </button>
            <a target="_blank" rel="noopener noreferrer" href={`https://siivagunner.fandom.com/wiki/${encodeURIComponent(rip.rawname.replace("#", ""))}`} class="btn">
                <OpenInNew />
            </a>
        </div>
    </div>
